...
/// Executes all requests in a folder
/// Note: Folder support depends on how folder metadata is stored in requests
Future<List<FolderRequestResult>> executeFolder(
  ...
  try {
    ...
    // For now, execute all requests in the collection
    // Folder filtering would require folder metadata in request items
    final folderRequests = requests; // TODO: Filter by folderId when folder support is added

    if (folderRequests.isEmpty) {
      throw Exception('Folder "$folderId" not found in collection "$collectionId" or is empty');
    }
    ...
    final results = <FolderRequestResult>[];

    for (final request in folderRequests) {
      ...
      if (error != null) {
        results.add(FolderRequestResult(
          requestId: execId,
          requestName: requestName,
          success: false,
          error: error,
...
/// Registers the exec_folder tool
void registerExecFolderTool(
  Server server, {
  required StorageService storage,
}) {
  server.addTool(
    name: 'exec_folder',
    description: 'Execute all requests in a specific folder within a collection',
    inputSchema: {
      'type': 'object',
      'properties': {
        'collection_id': {
          'type': 'string',
          'description': 'The collection ID containing the folder',
        },
        'folder_id': {
          'type': 'string',
          'description': 'The folder ID to execute',
        },
        'environment': {
          'type': 'string',
    ...
    handler: (arguments) async {
      try {
        ...
        final folderId = arguments['folder_id'] as String;
        final environment = arguments['environment'] as String?;

        final results = await executeFolder(
          storage,
          collectionId,
          folderId,
